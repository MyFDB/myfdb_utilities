...
== Installation as a gem from github:
  ...
  gem install myfdb_utilities-0.0.5.gem

== Settings

Run myfdb_utilities/bin/myfdb init
...
If you choose to install the cron task, you're done. To upload images, place a folder of images in ~/MyFDB_Uploads.

If you do NOT choose to install the cron task, you can upload the images in ~/MyFDB_Uploads by running myfdb_utilities/bin/myfdb upload_images .
...
Each folder is uploaded as a separate, newly created issue. If you want to upload to an already existing issue, just create a file in the folder called 'issue_id' containing the issue's id #.
...
To organize the images in a new, unpublished issue in the 'Title pending issues', click on 'edit' and fill in info on the issue. Hit the button at the bottom, and it will publish the issue and take you to a drag-n-drop screen where you can move the images into cover/campaign/editorial buckets. As you drag each image into the correct bucket, a modal window will open and prompt you to fill in the necessary info.
